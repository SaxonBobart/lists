...
- New task, event and note fields ask “Describe a task”, “Describe an event” and “Describe a note”. Existing-item editing remains literal.
- On supported devices with Apple Intelligence available, Foundation Models interprets new descriptions on device. The creation sheet fills editable fields after a typing pause while retaining the original description and caret. Explicit field edits take precedence. Inline creation interprets only its first editing session and preserves placement.
- Supported suggestions include title, supplied body, dates/times, duration, reminders, alarms, repeat rules, flags, priority, tags and existing destinations. Unsupported or ambiguous details fall back to manual entry; inference never invents destinations or changes item type.
- No model request sends descriptions to a hosted service. Unavailable models, unsupported languages, cancellation and timeout retain manual creation. Paste, duplicate and import do not trigger interpretation.
