...
/// sync (CONC-1: in-memory first, then persisted).
@MainActor
final class HabitCompletionStoreTests: XCTestCase {
    ...
    func testAddCompletionAppendsEventAndPersists() async throws {
        let (store, root, id) = try await storeWithHabit()
        try await store.addCompletion(id, at: .now)
        XCTAssertEqual(store.item(id)?.completions.count, 1)
        let reloaded = try await reload(root, id)
        XCTAssertEqual(reloaded?.completions.count, 1, "in-memory add must match disk")
    }

    func testDeleteCompletionRemovesByItsId() async throws {
        let (store, _, id) = try await storeWithHabit()
        try await store.addCompletion(id, at: .now)
        let cid = try XCTUnwrap(store.item(id)?.completions.first?.id)
        try await store.deleteCompletion(id, completionId: cid)
        XCTAssertEqual(store.item(id)?.completions.count, 0)
    }

    func testUpdateCompletionRetimesTheEvent() async throws {
        let (store, _, id) = try await storeWithHabit()
        ...
        XCTAssertEqual(store.item(id)?.completions.first?.at, t1)
    }

    func testRemoveLatestCompletionDropsTheMostRecentInThatCycle() async throws {
        let (store, _, id) = try await storeWithHabit()
        ...
        let comps = try XCTUnwrap(store.item(id)?.completions)
        XCTAssertEqual(comps.count, 1)
        XCTAssertEqual(comps.first?.at, early, "the −1 removes the latest event in the cycle")
    }

    func testIncrementHabitAddsAnEventAndCapsAtGoal() async throws {
        ...
        XCTAssertEqual(store.item(id)?.completions.count, 2)
    }

    func testAddCompletionsBulkAddsOnePerDateInOneWrite() async throws {
        let (store, root, id) = try await storeWithHabit()
        ...
        XCTAssertEqual(store.item(id)?.completions.count, 10, "one event per date in the range")
        let reloaded = try await reload(root, id)
        XCTAssertEqual(reloaded?.completions.count, 10, "bulk add must persist in a single write")
    }

    func testAddCompletionsWithNoDatesIsANoOp() async throws {
        let (store, _, id) = try await storeWithHabit()
        try await store.addCompletions(id, on: [])
        XCTAssertEqual(store.item(id)?.completions.count, 0)
    }

    func testSetHabitCountAddsAndTrimsEventsForACycle() async throws {
        ...
        XCTAssertEqual(store.item(id)?.completionLog["2026-05-15"], 3)
        try await store.setHabitCount(id, count: 1, on: day)
        XCTAssertEqual(store.item(id)?.completionLog["2026-05-15"], 1, "setting lower trims events")
    }
}
