...
- **UI automation works again (verified 2026-06-19):** XcodeBuildMCP's runtime-snapshot UI tools — `snapshot_ui`, `tap`, `swipe`, `drag`, `type_text`, `wait_for_ui`, `batch` — drive the simulator reliably under this toolchain. `snapshot_ui` returns an elementRef list (`e34|tap|button|Today, 9||sidebar.smartlist.today`) carrying the accessibility ID; tap by `elementRef`, and refresh the snapshot after any navigation/sheet change (refs go stale). This supersedes the old "AXe is permanently dead" note — the prior breakage cleared. (`open_sim` / `Simulator.app` are still gone — the device window is `DeviceHub.app`, and `build_run_sim` boots the sim for you.)
